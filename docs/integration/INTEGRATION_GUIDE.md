...
- Для table/journal screens используется общий паттерн `TableControlPanel`: отдельная строка поиска (если поиск есть) + отдельная строка filters/actions.
- Форматирование доменных label-ов (тип клиента, резидентство, комплаенс, вид договора, статус счёта, статус поручения) должно выполняться через `src/utils/labels.ts`.
- Локальные мапперы для этих доменных значений не рекомендуются: новые экраны должны переиспользовать централизованные formatter-функции.
