...
`static-uikit/assets/css/crm-static.bundle.css` уже сгенерирован как вспомогательный readable-бандл для удобства UMI/PHP runtime, если нужен один app CSS-файл. Источник истины остаётся в `crm-static.css` и импортируемых им модульных CSS; бандл нельзя редактировать вручную для feature/style work — после изменений в модульных CSS его нужно пересобирать.
...
- **Нельзя** редактировать `crm-static.bundle.css` вручную для feature/style work; после изменений в модульных CSS бандл нужно пересобирать.
...
### Генерируемый артефакт (runtime convenience, не источник истины)

```
static-uikit/assets/css/crm-static.bundle.css   ← генерируемый runtime convenience artifact
```

### Правило

> **Не редактировать `crm-static.bundle.css` вручную для feature/style work.**
> Редактировать только модульные CSS-файлы, затем пересобирать бандл после изменений.

### Текущий статус бандла

- `crm-static.bundle.css` существует только для удобства UMI/PHP runtime, когда один app CSS-файл предпочтительнее цепочки `@import`.
- Источник истины — `crm-static.css` плюс модульные CSS-файлы, которые он импортирует.
- Для feature/style work нельзя редактировать `crm-static.bundle.css` вручную; нужно менять исходные CSS-модули и затем пересобирать бандл.
- Бандл должен содержать только app CSS из `crm-static.css` и исключать `uikit.min.css`, `pages/auth.css` и все `document-templates/*.css`.
...
## 9. Стратегия генерации/пересборки бандла
...
| UMI PHP-разработчик редактирует бандл вручную для feature/style work | Высокое | Чётко пометить `crm-static.bundle.css` как runtime convenience artifact и «не редактировать вручную для feature/style work» |
...
| Риск | Нулевой |
| Критерии приёмки | Первые строки бандла содержат стабильный комментарий: «Инвестика CRM static UI — generated CSS bundle», источник истины, запрет ручных feature/style edits и пересборка после изменений модульных CSS |
