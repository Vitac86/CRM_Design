...
**Implementation status:** CSS cleanup Tasks A-D2 completed; Batch 2 registry table/card class-combination cleanup completed (2026-05-13); Batch 3 filter control class-combination cleanup completed (2026-05-13).

Batch 3 filter control class-combination cleanup status note (2026-05-13): DONE - Normal filter control wrappers across 9 pages (subjects, agents, archive, brokerage, compliance, requests, trading, trust-management, search-results) reduced from 3-class `crm-filter-control crm-filter-field crm-filter-pill-control` to 2-class `crm-filter-control crm-filter-pill-control`. Removed class: `crm-filter-field` (21 instances total). Rationale: all target pages override `.crm-filter-fields-row` to `display: flex` in `registry.css`/page CSS, making `crm-filter-field`'s `grid-column: span 3` inert; sizing is fully handled by `crm-filter-pill-control` selectors in `registry.css`. JS-contract filter classes (`crm-filter-panel`, `crm-registry-filters`, `crm-date-field`, `crm-filter-date-control`) preserved. Date controls in archive.html left untouched. No CSS files changed; no bundle regeneration needed. No JS files changed. All `data-*` hooks preserved.
